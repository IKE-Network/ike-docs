...
Full transitive dependency graph for `ike-doc-maven-plugin` 60, generated from [bom.json](bom.json)[1] (CycloneDX 1.6) at build time. Same SBOM source as the SPDX-grouped [licenses.html](licenses.html)[2] and the curated [built-with.html](built-with.html)[3] — three views of the same data.
...
| `network.ike.docs` | `koncept-asciidoc-extension` | `60` | `Apache-2.0` | library |
...
- As a Maven artifact: pull `ike-doc-maven-plugin:​60` with `<classifier>cyclonedx</classifier><type>json</type>` from Nexus / Maven Central.
